...
import { BookOpen, Car, TrendingUp, CheckCircle, XCircle } from 'lucide-react';
import {
...
export function DashboardPage() {
  ...
  // Chart data — last 14 days from the daily breakdown
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
        ...
        <StatCard
          ...
          sub={`${summary ? Math.round((summary.completed / (summary.total || 1)) * 100) : 0}% completion rate`}
        ...
        <StatCard
          ...
          sub={`${revenue?.paid_bookings ?? 0} paid bookings`}
        ...
        <StatCard
          ...
          sub={`${summary ? Math.round((summary.cancelled / (summary.total || 1)) * 100) : 0}% cancellation rate`}
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-6">
        {[
          ...
          <Card key={item.label} className="flex flex-col items-center py-5 gap-2">
      ...
      <Card padding={false} className="mt-6">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <a href="/bookings" className="text-xs text-brand-600 hover:underline font-medium">View all →</a>
        </div>
          ...
          {(recentBookings?.data ?? []).slice(0, 6).map(b => (
            <div key={b.id} className="flex items-center gap-4 px-6 py-3">
              ...
              <div className="shrink-0 text-right">
                ...
                <p className="text-[10px] text-slate-400 mt-1">{formatDateTime(b.created_at)}</p>
              </div>
            </div>
          ))}
        </div>
