...
import { zonesApi, vehicleTypesApi } from '../../api';
...
export function ZonesPage() {
  ...
  const { data: pricingData } = useQuery({
    queryKey: ['zone-pricing', pricingZone?.id],
    queryFn: () => zonesApi.getPricing(pricingZone!.id),
    enabled: !!pricingZone,
  });
  ...
  const zoneMutation = useMutation({
    mutationFn: () => editZone
      ? zonesApi.update(editZone.id, zoneForm)
      : zonesApi.create(zoneForm),
    onSuccess: () => {
      toast(editZone ? 'Zone updated.' : 'Zone created.', 'success');
  ...
  const setPricingMutation = useMutation({
    mutationFn: () => zonesApi.setPricing(pricingZone!.id, {
      vehicle_type_id: pricingForm.vehicle_type_id,
      base_fare:    parseFloat(pricingForm.base_fare),
      per_km_rate:  parseFloat(pricingForm.per_km_rate),
      per_stop_fee: parseFloat(pricingForm.per_stop_fee),
    }),
    onSuccess: () => {
      toast('Pricing saved.', 'success');
      qc.invalidateQueries({ queryKey: ['zone-pricing', pricingZone?.id] });
      setPricingForm({ vehicle_type_id: '', base_fare: '', per_km_rate: '', per_stop_fee: '' });
    },
    onError: (e: Error) => toast(e.message, 'error'),
  });
  ...
  const deletePricingMutation = useMutation({
    ...
    onSuccess: () => {
      toast('Pricing entry removed.', 'success');
      qc.invalidateQueries({ queryKey: ['zone-pricing', pricingZone?.id] });
  ...
  return (
    ...
    >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-1.5 mt-0.5">
                    {z.is_default ? <Badge className="bg-indigo-100 text-indigo-700">Default</Badge> : null}
                    <Badge status={z.is_active ? 'active' : 'inactive'}>{z.is_active ? 'Active' : 'Inactive'}</Badge>
                  </div>
                </div>
              </div>
              <div className="flex gap-1">
                <button onClick={() => openEditZone(z)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-brand-600 transition-colors">
                  <Pencil size={13} />
                </button>
                {!z.is_default && (
                  <button onClick={() => setDeleteZone(z)} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors">
                    <Trash2 size={13} />
                  </button>
                )}
              </div>
      ...
      <Modal
        ...
        title={editZone ? `Edit Zone — ${editZone.name}` : 'New Zone'}
        size="sm"
        footer={
          <>
            <Button variant="outline" onClick={() => setZoneModal(false)}>Cancel</Button>
            <Button loading={zoneMutation.isPending} onClick={() => zoneMutation.mutate()}>
      ...
      <ConfirmModal
        ...
        title="Delete Zone"
        message={`Delete zone "${deleteZone?.name}" and all its pricing entries?`}
        confirmLabel="Delete"
        danger
      ...
      <Modal
        open={!!pricingZone}
        onClose={() => setPricingZone(null)}
        title={`Pricing — ${pricingZone?.name}`}
        size="lg"
      >
        {/* Existing pricing */}
          ...
          <div className="mb-5">
            ...
            <div className="space-y-2">
              {pricingData!.pricing.map(p => (
                <div key={p.id} className="flex items-center gap-3 rounded-lg bg-slate-50 p-3">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-slate-800">{p.vehicle_type_name}</p>
                    <p className="text-xs text-slate-400 mt-0.5">
                      Base {formatCurrency(p.base_fare)} · /km {formatCurrency(p.per_km_rate)} · /stop {formatCurrency(p.per_stop_fee)}
                    </p>
                  </div>
                  <button
                    onClick={() => setDeletePricing({ zoneId: pricingZone!.id, pricing: p })}
        ...
        <div className="border-t border-slate-200 pt-4">
          ...
          <div className="space-y-3">
            <Select
              ...
              onChange={e => setPricingForm(p => ({ ...p, vehicle_type_id: e.target.value }))}
            ...
            <Button
              loading={setPricingMutation.isPending}
              disabled={!pricingForm.vehicle_type_id}
      ...
      {/* Delete pricing entry confirm */}
      <ConfirmModal
        open={!!deletePricing}
        onClose={() => setDeletePricing(null)}
        onConfirm={() => deletePricingMutation.mutate()}
        title="Remove Pricing Entry"
        message={`Remove pricing for "${deletePricing?.pricing.vehicle_type_name}"?`}
        confirmLabel="Remove"
        danger
