...
class MapsService {
  ...
  //
  // Strategy:
  //   • Web    → call Google Maps Geocoding REST API directly from the browser.
  //              The backend proxy won't work in dev (XAMPP has no internet).
  //   • Native → use the OS geocoder via the `geocoding` package.
  //              No API key, no network through our server, works offline.
  //              Falls back to direct Google API call if OS geocoder fails.

  static Future<String?> reverseGeocode(double lat, double lng) async {
    debugPrint('[MapsService] reverseGeocode($lat, $lng)  kIsWeb=$kIsWeb');
    if (kIsWeb) {
  ...
  /// Native path — OS geocoder via `geocoding` package.
  static Future<String?> _reverseGeocodeNative(double lat, double lng) async {
    try {
      ...
      debugPrint('[MapsService] placemarkFromCoordinates → ${marks.length} result(s)');
      if (marks.isEmpty) return _reverseGeocodeGoogleApi(lat, lng);

      final p = marks.first;
      debugPrint('[MapsService] placemark: name="${p.name}" street="${p.street}" '
          'sub="${p.subLocality}" locality="${p.locality}" '
          'state="${p.administrativeArea}" country="${p.country}"');

      final parts = <String>[
        // "name" is often the POI name; only include if it differs from street
      ...
      debugPrint('[MapsService] reverseGeocode (native) → "$addr"');
      return addr;
    } catch (e) {
      debugPrint('[MapsService] reverseGeocode (native) threw: $e — trying direct API');
      return _reverseGeocodeGoogleApi(lat, lng);
    }
  }

  /// Direct Google Maps Geocoding REST API call.
  /// Used on web (backend proxy can't reach Google in dev).
  /// Also used as native fallback when OS geocoder fails.
  static Future<String?> _reverseGeocodeGoogleApi(double lat, double lng) async {
    final key = AppConfig.googleMapsKey;
    if (key.isEmpty) {
      debugPrint('[MapsService] _reverseGeocodeGoogleApi: no API key configured');
      return null;
    }
    try {
      debugPrint('[MapsService] _reverseGeocodeGoogleApi: calling Google directly');
      // Use a plain Dio instance — not the backend proxy client.
      final resp = await Dio().get<Map<String, dynamic>>(
        'https://maps.googleapis.com/maps/api/geocode/json',
        queryParameters: {'latlng': '$lat,$lng', 'key': key},
      );
      final results = resp.data?['results'] as List?;
      debugPrint('[MapsService] _reverseGeocodeGoogleApi: ${results?.length ?? 0} result(s)');
      ...
      for (final r in results) {
        ...
        if (types.contains('street_address') ||
          ...
          debugPrint('[MapsService] _reverseGeocodeGoogleApi → "$addr" (type: $types)');
          return addr;
        }
      }
      final addr = (results.first as Map)['formatted_address'] as String?;
      debugPrint('[MapsService] _reverseGeocodeGoogleApi (first result) → "$addr"');
      return addr;
    } catch (e, st) {
      debugPrint('[MapsService] _reverseGeocodeGoogleApi threw: $e\n$st');
      return null;
    }
  }
